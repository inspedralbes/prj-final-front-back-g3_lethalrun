<template>
  <div class="min-h-screen w-full flex flex-col" style="background-image: linear-gradient(to bottom, #1a0a0a 0%, #12122a 100%);">
    <Navbar class="h-min" :logoSrc="'/LethalRun_logo-removebg-preview.png'" :logoLink="'/'" :menuItems="menuItems"
      ...
      :isLogged="isLogged" />
    <div class="py-8 h-full flex-grow overflow-hidden">
      <div id="stars"></div>
      <div id="stars2"></div>
      <div id="stars3"></div>
      <div id="stars4"></div>
      ...
      <div id="contt" class="mx-auto max-w-7xl h-full px-4 sm:px-6 lg:px-8 flex flex-col">
        ...
        <div v-if="graffitis.length > 0" class="pt-8 h-full w-full ">
          <div
            class="flex flex-col gap-4 xs:flex-row sm:flex-row md:flex-row lg:flex-row xl:flex-row 2xl:flex-row justify-between items-center mb-6">
            ...
            <div class="flex justify-center items-center gap-4">
              ...
              <button
                ...
                :class="[
                  'px-4 py-2 rounded-md font-medium transition-all duration-300 border shadow-md',
                  currentPage === 1
                    ? 'bg-gray-700 text-gray-400 cursor-not-allowed border-gray-600 opacity-50'
                    : 'bg-gray-800 text-white hover:bg-gray-700 cursor-pointer border-gray-500 hover:shadow-[0_0_10px_rgba(75,85,99,0.6)]'
                ]">
                &lt;
              </button>
            ...
            <button @click="openModal"
              class="cursor-pointer text-white p-2 px-4 rounded-lg shadow-md font-medium truncate transition-all duration-300 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-4 w-full">
              <div v-for="graffiti in paginatedGraffitis" :key="graffiti.id"
                ...
                <div class="w-full container-buttons flex justify-between items-center">
                  ...
                  <p v-else
                      class="font-medium cursor-default truncate text-white p-2 px-3 rounded-lg shadow-md transition-all duration-300 hover:brightness-110"
                      style="background-color: #16a34a;">
                      Activa
                  </p>


                  <button @click="deleteGraffitiHandler(graffiti.id, graffiti.path)" :disabled="graffiti.is_active" 
      ...
      <div v-if="isModalOpen"
        ...
        <div class="rounded-xl shadow-2xl w-4xl max-w-full relative flex flex-col max-h-[90vh]" style="background: rgba(30, 20, 20, 0.98); border: 1px solid rgba(255,255,255,0.1);">
          ...
          <div class="overflow-y-auto px-6 py-4">
            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 w-full gap-6 justify-center">

              <div class="flex flex-col justify-start items-center w-full space-y-4">
                <button @click.prevent="showFileChooser"
                  class="cursor-pointer w-full py-3 px-4 rounded-lg shadow-sm focus:ring-yellow-400 focus:border-yellow-400 text-center font-medium hover:opacity-90 transition"
                ...
                <div v-if="imageSrc" class="w-full border border-gray-700 rounded-lg overflow-hidden">
                  <vue-cropper ref="cropper" class="w-full h-auto max-w-full" :src="imageSrc" :aspect-ratio="1"
                     :guides="true" :view-mode="2" :auto-crop-area="0.8" :background="false" :drag-mode="'move'"
                     style="image-orientation: from-image; background-color: #2d3748;"
                     :cropBoxResizable="true" :cropBoxMovable="true">
                  </vue-cropper>
                </div>
                 <div v-else class="w-full aspect-square bg-gray-700/50 border border-dashed border-gray-600 rounded-lg flex items-center justify-center text-gray-500">
                    Aquí es mostrarà la imatge per retallar
                </div>


                <button v-if="imageSrc" @click="cropImage"
                  class="cursor-pointer w-full py-3 px-4 rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-1 font-medium transition hover:opacity-90"
